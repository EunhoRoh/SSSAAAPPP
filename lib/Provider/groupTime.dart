...
class GroupTime {
  ...
  Future<int> addGroupSchedule(
    ...
    available.add(GroupTimeData(
        startTime: start,
        endTime: end,
        duration: Duration(minutes: end.difference(start).inMinutes))); //이거 오류
      ...
      for (final avatime in available) {
        ...
        if (schedule.from.isAfter(avatime.endTime)) {
          ...
          tem.add(GroupTimeData(
              ...
              duration: Duration(
                  minutes:
                      schedule.from.difference(avatime.startTime).inMinutes)));
          tem.add(GroupTimeData(
              startTime: schedule.to,
              endTime: avatime.endTime,
              duration: Duration(
                  minutes: avatime.endTime.difference(schedule.to).inMinutes)));
          //print('사이');
        }
        ...
        //스케줄 시간이 가능시간의 끝시간을 포함 한 경우 => 가능시간의 끝시간을 스케줄의 시작시간으로 변경
          ...
          avatime.duration = Duration(
              minutes: schedule.from.difference(avatime.startTime).inMinutes);
          tem.add(avatime);
          //print('끝');
        }
    ...
    print(
        'all available time for meeting after filtering: ${available.length}');
